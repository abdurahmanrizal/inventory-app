...
export default function Index({ transactions, warehouses, items, userWarehouse, access }) {
  ...
  const changeType = (type) =>
    form.setData({ ...form.data, type, destination_warehouse_id: "" });
  const submit = (event) => {
  ...
  return (
    <AppLayout title="Stock Out & Mutasi">
      <Head title="Stock Out & Mutasi" />
      ...
      <div className="mb-6">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
          Pilih Stock Out untuk barang keluar dari perusahaan, atau Mutasi untuk
      ...
      <form onSubmit={submit} className="space-y-5">
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.03)]">
          <div className="border-b border-slate-100 p-5 sm:px-6">
            ...
            <div className="mt-3 grid max-w-lg grid-cols-2 gap-2 rounded-xl bg-slate-100 p-1.5">
              {access.canTransfer && <button
                type="button"
                onClick={() => changeType("stock_out")}
                className={`rounded-lg px-4 py-2.5 text-sm font-semibold transition ${!isTransfer ? "bg-white text-slate-950 shadow-sm" : "text-slate-500 hover:text-slate-800"}`}
              >
              ...
              <button
                ...
                className={`rounded-lg px-4 py-2.5 text-sm font-semibold transition ${isTransfer ? "bg-white text-slate-950 shadow-sm" : "text-slate-500 hover:text-slate-800"}`}
              >
                <ArrowRightLeft size={16} className="mr-2 inline" />
                Mutasi
              </button>
            </div>
          </div>
          ...
          <div className="grid gap-5 p-5 sm:p-6 md:grid-cols-2 xl:grid-cols-4">
            <label className="space-y-2">
              ...
              >
                <option value="">Pilih gudang asal</option>{warehouses.map((warehouse)=><option key={warehouse.id} value={warehouse.id}>{warehouse.name}</option>)}
              </select> : <div className={`${fieldClass} flex items-center bg-slate-50 text-slate-600`}>{userWarehouse?.name || "Gudang akun belum ditentukan"}</div>}
            </label>
            {isTransfer ? (
              <label className="space-y-2">
                <span className="text-xs font-semibold text-slate-700">
                ...
                >
                  ...
                  {warehouses
                    .filter(
                      (warehouse) =>
                        String(warehouse.id) !==
                        String(form.data.source_warehouse_id),
                    )
                    .map((warehouse) => (
              ...
              <label className="space-y-2">
                <span className="text-xs font-semibold text-slate-700">Jenis pengeluaran <b className="text-rose-500">*</b></span>
                <select className={fieldClass} value={form.data.stock_out_reason} onChange={(event)=>form.setData("stock_out_reason",event.target.value)}>
                  <option value="operational">Pemakaian operasional</option><option value="shrinkage">Penyusutan</option><option value="expired">Kedaluwarsa</option><option value="damaged">Rusak</option><option value="waste">Waste / terbuang</option><option value="return">Retur</option><option value="other">Lainnya</option>
                </select>
              </label>
            )}
            {!isTransfer && (
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.03)]">
          ...
          <div className="flex flex-col-reverse gap-3 border-t border-slate-100 bg-slate-50/50 px-5 py-4 sm:flex-row sm:justify-end sm:px-6">
            ...
            >
              ...
              {form.processing
                ? "Mengirim..."
                : `Ajukan ${isTransfer ? "Mutasi" : "Stock Out"}`}
            </button>
          </div>
        </section>
      </form>
      ...
      <TransactionHistory
        ...
        title="Daftar pengajuan Stock Out & Mutasi"
        emptyText="Pengajuan Stock Out atau Mutasi yang dibuat akan muncul di sini."
      />
      <ConfirmActionDialog
        open={confirmOpen}
        ...
        tone="amber"
        title={`Ajukan ${isTransfer ? "mutasi" : "Stock Out"}?`}
        description="Pastikan gudang, produk, jumlah, dan batch sudah benar. Transaksi akan masuk ke antrean approval."
