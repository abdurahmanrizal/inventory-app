...
class DashboardController extends Controller
{
    public function __invoke(Request $request)
    {
        ...
        $stockValue = (clone $stockQuery)->selectRaw('COALESCE(SUM(qty_on_hand * average_cost), 0) AS total')->value('total');
        $legacyPending = StockTransaction::query()
            ->where('status', TransactionStatus::WaitingApproval)
            ->when(! $isSuperadmin, $transactionScope)
            ->count();
        $requestPending = StockRequest::query()
            ->where('status', 'waiting_approval')
            ->when(! $isSuperadmin, $requestScope)
            ->count();
